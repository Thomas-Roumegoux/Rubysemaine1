...
@journalistes.each do |journaliste|
    if journaliste.index(/[0123456789]/)
        @n = @n + 1 
    end
end
end
...
@a = 0
@journalistes.each do |journaliste|
    if journaliste.downcase.index("aude")
        @a = @a + 1 
    end
end
end

def combien_capitalize
@c = 0
@journalistes.each do |s|
    if s[1].index(/[A-Z]/)
        @c = @c + 1 
    end
end
...
@m = 0
@journalistes.each do |journaliste|
    if journaliste.index(/[A-Z]/)
        @m = @m + 1 
    end
end
end

def combien_underscore
@u = 0
@journalistes.each do |journaliste|
    if journaliste.index("_")
        @u = @u + 1
    end
end
end
...
l = @journalistes.length
p "Il y'a #{l} journalistes"
combien_handdle
p "Combien d'handle contiennent un numero? ==> #{@n}"
combien_aude
p "Combien d'handle contiennent les 4 lettres du prénom Aude à la suite ? ==> #{@a}"
combien_capitalize
p "Il y'a #{@c} handle qui commencent par une majuscule"
combien_maj
p "Il y'a #{@m} handle qui contiennent une majuscule"
combien_underscore
p "Il y'a #{@u} handle qui contiennent un underscore"
s = @journalistes.sort
p "#{s}"
